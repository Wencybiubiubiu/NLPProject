...
def get_embedding_form(wrapped_dict,embedding_size,embedding_way_for_diff_length_sentence,Max_padding_size,text_embedding,issue_embedding):
    ...
    if use_pretrained:
        # text_embedding.from_pretrained(torch.load(vocab_file))
        text_to_embedding = pretrained_embedding
...
def execute(data_format,model_type,train_ratio,validation_ratio,test_ratio,
    ...
    train_tweet_id2text.update(unlabeled_text)
    train_tweet_id2label.update(unlabeled_label)
    train_tweet_id2issue.update(unlabeled_issue)
    '''
    Implement your Neural Network classifier here
    '''
...
if __name__ == '__main__':
    ...
    number_of_epoches = 20 #300
    whether_analyze = 1 #1:yes,0:no
    num_of_record_results = 20
    embedding_size = 300
    ...
    if use_pretrained:
        ...
        pretrained_embedding = torch.load(vocab_file)
        print('Pre-trained word2vec loaded')
    ...
    if(compare_two_models == True):
        ...
        plt.legend()
        #plt.savefig('F1score_diff_lr_op'+str(args.option)+'_sgd.png')
        plt.savefig('compare_two_models_bow_word2vec_epoch20_1.png')
        plt.close()
        exit(0)
