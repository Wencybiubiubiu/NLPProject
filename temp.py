...
author_label = 'author'
label = 'label'
x_matrix, y_matrix, key_list, label_list, label_type_list = None,None,None,None,None
...
def wrap_data_dict(preprocessed_text, train_tweet_id2issue, train_tweet_id2author_label, train_tweet_id2label):
    ...
    for key in preprocessed_text:
        ...
        cur_dict[author_label] = train_tweet_id2author_label[key]
...
def get_preprocessed_x_matrix(wrapped_dict):
    ...
    for key in wrapped_dict:
        ...
        corresponding_author = wrapped_dict[key][author_label]
        corresponding_label = int(wrapped_dict[key][label])-1

        corresponding_x_i = np.concatenate((corresponding_text, corresponding_issue,corresponding_author))
...
def get_all_dict_to_ix(wrapped_dict):
    ...
    author_word_dict_to_ix = create_word_dict(wrapped_dict,author_label)

    return text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix

def BoW(wrapped_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix):
    ...
    wrapped_dict = one_hot_encoding_for_dict(wrapped_dict,author_label,author_word_dict_to_ix)
...
def wrap_dict_to_separate_list(wrapped_dict):
    ...
    for key in wrapped_dict:
        ...
        author_list.append(wrapped_dict[key][author_label])
        key_list.append(key)
    ...
    return text_list,issue_list,author_list,key_list,label_list,label_type_list
...
def handle_data_with_word_embedding(text_embedding_list,issue_embedding_list,author_embedding_list,
    label_list,embedding_way_for_diff_length_sentence):
    ...
    for i in range(len(text_embedding_list)):
        ...
        cur_author_embedding_list = author_embedding_list[i]
        ...
        for j in range(len(cur_author_embedding_list)):
            #count+=1
            #cur_context = cur_context+cur_author_embedding_list[j]
            cur_context = torch.cat((cur_context, cur_author_embedding_list[j]),1)
...
def get_embedding_form(wrapped_dict,embedding_size,embedding_way_for_diff_length_sentence,text_embedding,
    issue_embedding,author_embedding,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix):
    ...
    author_ix_to_embedding = embeddings_lookup_table(author_word_dict_to_ix,len(author_word_dict_to_ix),embedding_size,author_embedding)
    ...
    author_to_embedding = word_to_embedding(author_word_dict_to_ix,author_ix_to_embedding)


    text_list,issue_list,author_list,key_list,label_list,label_type_list = wrap_dict_to_separate_list(wrapped_dict)
    ...
    author_embedding_list = wrap_embedding_training_set(author_list,author_to_embedding)

    context,label_index = handle_data_with_word_embedding(text_embedding_list,issue_embedding_list,author_embedding_list,label_list,
    ...
    #print(context.shape)
    #exit()
    return context,label_index,key_list,label_list,label_type_list

def handle_data_with_word_embedding_2d(text_embedding_list,issue_embedding_list,author_embedding_list,
    label_list,embedding_way_for_diff_length_sentence):
    ...
    for i in range(len(text_embedding_list)):
        ...
        for j in range(len(cur_author_embedding_list)):
            #count+=1
            #cur_context = cur_context+cur_author_embedding_list[j]
            cur_context = torch.cat((cur_context, cur_author_embedding_list[j]),0)
...
def get_embedding_form_2d(wrapped_dict,embedding_size,embedding_way_for_diff_length_sentence):

    text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix = get_all_dict_to_ix(wrapped_dict)
    ...
    author_embedding = nn.Embedding(len(author_word_dict_to_ix), embedding_size)
    ...
    context,label_index = handle_data_with_word_embedding_2d(text_embedding_list,issue_embedding_list,author_embedding_list,label_list,
        embedding_way_for_diff_length_sentence)
...
def get_bow_embedding_form(wrapped_dict,embedding_size,embedding_way_for_diff_length_sentence):
    ...
    author_encoding_list = []
    for i in range(len(author_list)):
        author_encoding_list.append([torch.FloatTensor([one_hot_encoding_for_one_vector(author_list[i],author_word_dict_to_ix)])])

    context,label_index = handle_data_with_word_embedding(text_embedding_list,issue_encoding_list,author_encoding_list,label_list,
        embedding_way_for_diff_length_sentence)
...
def get_embedding_form_Doc2Vec(wrapped_dict,embedding_size,text_vector_dict,embedding_way_for_diff_length_sentence):
    ...
    context,label_index = handle_data_with_word_embedding(text_vector_list,issue_embedding_list,author_embedding_list,label_list,
        embedding_way_for_diff_length_sentence)
...
class NeuralNet(nn.Module):  
    ...
    def __init__(self, total_embedding_size, label_num,
        data_format,wrapped_train_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix,
        embedding_way_for_diff_length_sentence,copied_train_text,output_dict):
        ...
        if(data_format == 'BoW'):
            x_matrix, y_matrix, key_list, label_type_list = BoW(wrapped_train_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix)
        elif(data_format == 'WordEmbedding'):

            text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix = get_all_dict_to_ix(wrapped_train_dict)

            text_embedding = nn.Embedding(len(text_word_dict_to_ix), embedding_size)
            issue_embedding = nn.Embedding(len(issue_word_dict_to_ix), embedding_size)
            author_embedding = nn.Embedding(len(author_word_dict_to_ix), embedding_size)
            x_matrix, y_matrix, key_list, label_list, label_type_list = get_embedding_form(wrapped_train_dict,embedding_size,embedding_way_for_diff_length_sentence,
                text_embedding,issue_embedding,author_embedding,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix)
        elif(data_format == 'NoFlattenEmbedding'):
            x_matrix, y_matrix, key_list, label_list, label_type_list = get_embedding_form_2d(wrapped_train_dict,embedding_size,embedding_way_for_diff_length_sentence)
        elif(data_format == 'BoW_WordEmbedding'):
            x_matrix, y_matrix, key_list, label_list, label_type_list = get_bow_embedding_form(wrapped_train_dict,embedding_size,embedding_way_for_diff_length_sentence)
        elif(data_format == 'Doc2Vec'):
            text_vector_dict = get_sentence_vector_lookup_table(copied_train_text,embedding_size)
            x_matrix, y_matrix, key_list, label_list, label_type_list = get_embedding_form_Doc2Vec(wrapped_train_dict,embedding_size,text_vector_dict,embedding_way_for_diff_length_sentence)
        else:
            print("Please set a feasible data pre-processing method: BoW, WordEmbedding, ...")

        output_dict['x_matrix'] = x_matrix
        output_dict['y_matrix'] = y_matrix
        output_dict['key_list'] = key_list
        output_dict['label_list'] = label_list
        output_dict['label_type_list'] = label_type_list
        print(x_matrix, y_matrix, key_list, label_list, label_type_list)
    def forward(self, x):
...
def train(train_ratio,validation_ratio,test_ratio,learning_rate,weight_decay_rate,number_of_epoches,
    embedding_size,whether_analyze,num_of_record_results,model_type,loss_function_choice,update_method,
        data_format,wrapped_train_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix,
        embedding_way_for_diff_length_sentence,copied_train_text):
    ...
    if(model_type == 'NN'):
        output_dict = {}
        model = NeuralNet(num_of_each_x,num_of_labels,
        data_format,wrapped_train_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix,
        embedding_way_for_diff_length_sentence,copied_train_text,output_dict)

        x_matrix = output_dict['x_matrix'] 
        y_matrix= output_dict['y_matrix']  
        key_list = output_dict['key_list'] 
        label_list = output_dict['label_list'] 
        label_type_list = output_dict['label_type_list'] 
    elif(model_type == 'NN2'):
        model = NeuralNet_2(num_of_each_x,num_of_labels)
    elif(model_type == 'CNN'):
        model = CNN(embedding_size, 100, [3,4,5], num_of_labels, 0.5)
    else:
...
def execute(data_format,model_type,train_ratio,validation_ratio,test_ratio,
    ...
    wrapped_train_dict = wrap_data_dict(handle_text(train_tweet_id2text), handle_text(train_tweet_id2issue), handle_text(train_tweet_id2author_label), train_tweet_id2label)
    text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix = get_all_dict_to_ix(wrapped_train_dict)
    
    model,train_accu_array,validation_accu_array,epoch_num_array = train(train_ratio,validation_ratio,test_ratio,learning_rate,weight_decay_rate,number_of_epoches,
        embedding_size,whether_analyze,num_of_record_results,model_type,loss_function_choice,update_method,
        data_format,wrapped_train_dict,text_word_dict_to_ix,issue_word_dict_to_ix,author_word_dict_to_ix,
        embedding_way_for_diff_length_sentence,copied_train_text)
    
    if(whether_analyze==1):
        accuracy_bow_nn_fileName = image_folder + '/' + data_format + str(embedding_size) + '_' + model_type \
            + '_' + update_method + '_' + 'lr' + str(learning_rate) + '_' + 'epoch' \
            + str(number_of_epoches) 
        if(data_format == 'WordEmbedding' or data_format == 'NoFlattenEmbedding'):
            accuracy_bow_nn_fileName += '_' + embedding_way_for_diff_length_sentence
        plot_one_x_vs_two_y(epoch_num_array,train_accu_array,validation_accu_array,'num of epoch','accuracy','training accuracy','validation accuracy',accuracy_bow_nn_fileName)

    test_accuracy = evaluate(model,test_x,test_y)

    print("Test accuracy: " + str(test_accuracy))

    return epoch_num_array,train_accu_array,validation_accu_array
...
if __name__ == '__main__':
    ...
    num_of_record_results = 10
    embedding_size = 1000

    #data format can be chosen from: BoW, WordEmbedding, Doc2Vec,BoW_WordEmbedding,NoFlattenEmbedding
    data_format = 'WordEmbedding'
    #embedding_way_for_diff_length_sentence can be chosen from: 
    #Average, Sum, Min, Max, MinMax, PaddingByMax, CuttingByMin,Sentence
    embedding_way_for_diff_length_sentence = 'Sum'
    ...
    image_folder = 'final_image'
    if not os.path.exists(image_folder):
        os.makedirs(image_folder)

    first_epoch_num_array,first_train_accu_array,first_validation_accu_array = execute(data_format,model_type,train_ratio,validation_ratio,test_ratio,
        learning_rate,weight_decay_rate,number_of_epoches,embedding_size,
        whether_analyze,num_of_record_results,image_folder,update_method,
        loss_function_choice,embedding_way_for_diff_length_sentence)

    exit()
    data_format = 'NoFlattenEmbedding'
    embedding_way_for_diff_length_sentence = 'PaddingByMax'
    model_type = 'CNN'

    second_epoch_num_array,second_train_accu_array,second_validation_accu_array = execute(data_format,model_type,train_ratio,validation_ratio,test_ratio,
        learning_rate,weight_decay_rate,number_of_epoches,embedding_size,
        whether_analyze,num_of_record_results,image_folder,update_method,
        loss_function_choice,embedding_way_for_diff_length_sentence)
